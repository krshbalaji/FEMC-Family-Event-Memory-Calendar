...
class FEMCApi:
    ...
    def create_event_for_session(self, session_id: str, title: str, description: str, family_context_id: Optional[str], start_time: datetime.datetime, end_time: Optional[datetime.datetime], visibility: VisibilityLevel = VisibilityLevel.FAMILY, place_id: Optional[str] = None, recurrence_rule: Optional[RecurrenceRule] = None, category: EventCategory = EventCategory.GENERAL, target_person_ids: Optional[List[str]] = None, milestone_year: Optional[int] = None, milestone_anchor_date: Optional[datetime.date] = None) -> Event:
        session = self._validate_session(session_id)
        return self.event.create_event(owner_id=session.account_id, title=title, description=description, family_context_id=family_context_id, start_time=start_time, end_time=end_time, visibility=visibility, place_id=place_id, recurrence_rule=recurrence_rule, category=category, target_person_ids=target_person_ids, milestone_year=milestone_year, milestone_anchor_date=milestone_anchor_date)
    ...
    def configure_reminder_for_session(self, session_id: str, event_id: str, offset_minutes: int = 15, reminder_type: ReminderType = ReminderType.EVENT_START) -> ReminderConfig:
        session = self._validate_session(session_id)
        return self.reminder.create_reminder(created_by_id=session.account_id, event_id=event_id, offset_minutes=offset_minutes, reminder_type=reminder_type)
    ...
    def trigger_due_reminders_for_session(self, session_id: str, family_context_id: str, current_time: Optional[datetime.datetime] = None) -> List[Notification]:
        session = self._validate_session(session_id)
        return self.reminder.evaluate_due_reminders(account_id=session.account_id, family_context_id=family_context_id, current_time=current_time)
    ...
    def get_calendar_for_session(self, session_id: str, family_context_id: str, start_date: Optional[datetime.date] = None, end_date: Optional[datetime.date] = None):
        session = self._validate_session(session_id)
        return self.calendar.get_calendar_for_context(session.account_id, family_context_id, start_date=start_date, end_date=end_date)
    ...
    def create_memory_for_session(self, session_id: str, event_id: str, narrative: str, visibility: VisibilityLevel = VisibilityLevel.FAMILY) -> Memory:
        session = self._validate_session(session_id)
        event = self.canonical.get_event(event_id)
        if event is None:
            raise ValueError("Event does not exist")
        self.event.get_event_for_account(event_id, session.account_id)
        return self.memory.create_memory(subject_id=session.account_id, narrative=narrative, visibility=visibility, created_by_id=session.account_id, event_id=event_id)
    ...
    def create_relationship_for_session(self, session_id: str, source_person_id: str, target_person_id: str, relationship_type: RelationshipType = RelationshipType.MEMBER, confidence: Confidence = Confidence.MEDIUM) -> Relationship:
        session = self._validate_session(session_id)
        ...
        return self.identity.create_relationship(source_person_id=source_person_id, target_person_id=target_person_id, relationship_type=relationship_type, confidence=confidence)

    def get_family_topology_for_session(self, session_id: str, family_context_id: str) -> FamilyTopologyResult:
        session = self._validate_session(session_id)
        state = self.guided_experience.get_session(session.account_id)
        if state is not None and state.current_mode == GuideMode.LEARN_BY_DOING:
            pw = self.guided_experience.practice_worlds.get(session.account_id)
            if pw is not None:
                return self.guided_experience.get_practice_family_topology_projection(session.account_id, family_context_id, pw)
        return self.identity.get_family_topology_for_account(family_context_id, session.account_id)

    def create_place_for_session(self, session_id: str, name: str, address: str = "", family_context_id: Optional[str] = None, visibility: VisibilityLevel = VisibilityLevel.FAMILY) -> Place:
        session = self._validate_session(session_id)
        return self.place.create_place(created_by_id=session.account_id, name=name, address=address, family_context_id=family_context_id, visibility=visibility)
    ...
    def create_media_item_for_session(self, session_id: str, uri: str, media_type: MediaType = MediaType.PHOTO, caption: str = "", family_context_id: Optional[str] = None, event_id: Optional[str] = None, memory_id: Optional[str] = None, visibility: VisibilityLevel = VisibilityLevel.FAMILY) -> MediaItem:
        session = self._validate_session(session_id)
        return self.media.create_media_item(owner_id=session.account_id, uri=uri, media_type=media_type, caption=caption, family_context_id=family_context_id, event_id=event_id, memory_id=memory_id, visibility=visibility)
    ...
    def create_media_album_for_session(self, session_id: str, title: str, description: str = "", family_context_id: Optional[str] = None, media_ids: Optional[List[str]] = None, visibility: VisibilityLevel = VisibilityLevel.FAMILY) -> MediaAlbum:
        session = self._validate_session(session_id)
        return self.media.create_media_album(owner_id=session.account_id, title=title, description=description, family_context_id=family_context_id, media_ids=media_ids, visibility=visibility)
    ...
    def get_timeline_for_session(self, session_id: str, family_context_id: str, limit: Optional[int] = None) -> List[TimelineProjectionEntry]:
        session = self._validate_session(session_id)
        return self.timeline.get_timeline_for_family_context_for_account(family_context_id, session.account_id, limit=limit)

    def get_timeline_for_context_for_session(self, session_id: str, family_context_id: str, limit: Optional[int] = None) -> List[TimelineProjectionEntry]:
        return self.get_timeline_for_session(session_id, family_context_id, limit=limit)

    def list_media_for_session(self, session_id: str, family_context_id: str) -> List[MediaItem]:
        session = self._validate_session(session_id)
        return [m for m in self.canonical.list_media_items() if m.family_context_id == family_context_id]

    def generate_celebration_for_session(self, session_id: str, family_context_id: str, title: str = "Celebration", subtitle: str = "", content: str = "", template_id: str = "classic", visibility: VisibilityLevel = VisibilityLevel.FAMILY, artifact_type: CelebrationArtifactType = CelebrationArtifactType.EVENT_HIGHLIGHT, target_id: Optional[str] = None) -> CelebrationArtifact:
        session = self._validate_session(session_id)
        return self.celebration_studio.generate_artifact(created_by_id=session.account_id, family_context_id=family_context_id, title=title, subtitle=subtitle, content=content, template_id=template_id, visibility=visibility, artifact_type=artifact_type, target_id=target_id)

    def list_celebration_artifacts_for_session(self, session_id: str, family_context_id: str) -> List[CelebrationArtifact]:
        session = self._validate_session(session_id)
        return self.celebration_studio.list_celebration_artifacts(session.account_id, family_context_id)

    def create_share_link_for_session(self, session_id: str, resource_type: ShareResourceType, resource_id: str, family_context_id: str, expires_in_minutes: int = 1440) -> ShareLink:
        session = self._validate_session(session_id)
        return self.sharing.create_share_link(created_by_id=session.account_id, family_context_id=family_context_id, resource_type=resource_type, resource_id=resource_id, expires_in_minutes=expires_in_minutes)
    ...
    def resolve_share_link_for_session(self, session_id: str, token: str) -> ShareLink:
        session = self._validate_session(session_id)
        return self.sharing.resolve_share_link(token)

    def start_practice_world_for_session(self, session_id: str, family_context_id: str, context_type: ContextType = ContextType.FAMILY, age_group: AgeGroup = AgeGroup.MIXED, include_family: bool = True, language: Language = Language.ENGLISH) -> MayilPracticeWorld:
        session = self._validate_session(session_id)
        return self.guided_experience.get_or_create_practice_world(session.account_id, family_context_id, context_type, age_group, include_family, language)
    ...
    def execute_simulated_action_for_session(self, session_id: str, action_type: ActionType, control_id: str, resource_type: ResourceType = ResourceType.EVENT, payload: Optional[dict] = None) -> dict:
        session = self._validate_session(session_id)
        return self.guided_experience.execute_simulated_action(session.account_id, action_type, control_id, resource_type, payload)
    ...
    def get_members_projection(self, session_id: str, account_sessions: Dict[str, str], active_account_id: str):
        ...
        for acc_id, sess_id in account_sessions.items():
            ...
            if acc and per:
                members_list.append({"account_id": acc.id, "person_id": per.id, "name": per.name, "email": acc.email, "username": acc.username, "session_id": sess_id, "is_active": acc.id == active_account_id})
        return members_list
    ...
    def get_timeline_projection(self, session_id: str, family_context_id: str, default_event_id: str):
        ...
        return self.timeline.get_timeline_for_family_context_for_account(fc_id, session.account_id), self.get_event_with_memories_for_session(session_id, default_event_id)
    ...
    def get_export_projection(self, session_id: str, family_context_id: str):
        ...
        if pw is not None:
            ...
            return export_data, {"is_valid": True, "errors": [], "warnings": [], "record_counts": {}}
    ...
    def get_history_projection(self, session_id: str, family_context_id: str):
        ...
        for record in history:
            ...
            for key in ("action_type", "resource_type", "visibility"):
                value = item.get(key)
    ...
    def get_practice_calendar_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_calendar_projection(practice_world)

    def get_practice_event_detail_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_event_detail_projection(practice_world)

    def get_practice_timeline_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_timeline_projection(practice_world)

    def get_practice_media_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_media_projection(practice_world)

    def get_practice_celebrations_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_celebrations_projection(practice_world)

    def get_practice_sharing_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_sharing_projection(practice_world)

    def get_practice_export_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_export_projection(practice_world)

    def get_practice_history_projection(self, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_history_projection(practice_world)

    def get_practice_family_topology_projection(self, account_id: str, family_context_id: str, practice_world: MayilPracticeWorld):
        return self.guided_experience.get_practice_family_topology_projection(account_id, family_context_id, practice_world)

    def start_guided_experience_for_session(self, session_id: str, family_context_id: str, mode: GuideMode = GuideMode.WATCH_JOURNEY, context_type: ContextType = ContextType.FAMILY, age_group: AgeGroup = AgeGroup.MIXED, include_family: bool = True, language: Language = Language.ENGLISH):
        session = self._validate_session(session_id)
        return self.guided_experience.initialize_session(session.account_id, family_context_id, mode, context_type, age_group, include_family, language)

    def reset_guided_experience_for_session(self, session_id: str):
        session = self._validate_session(session_id)
    ...
    def switch_guided_experience_mode_for_session(self, session_id: str, mode: GuideMode):
        session = self._validate_session(session_id)
        return self.guided_experience.switch_mode(session.account_id, mode)

    def initialize_guided_experience_for_session(self, session_id: str, family_context_id: str, mode: GuideMode, context_type: ContextType, age_group: AgeGroup, include_family: bool, language: Language):
        session = self._validate_session(session_id)
        return self.guided_experience.initialize_session(session.account_id, family_context_id, mode, context_type, age_group, include_family, language)

    def get_shared_journey_scenes_for_session(self, session_id: str):
        session = self._validate_session(session_id)
        return self.guided_experience.journey_scenes

    def validate_guided_action_for_session(self, session_id: str, action_type: ActionType, control_id: str, resource_id: str = "", resource_label: str = "", operation: str = ""):
        session = self._validate_session(session_id)
        return self.guided_experience.validate_action(session.account_id, action_type, control_id, resource_id, resource_label, operation)
